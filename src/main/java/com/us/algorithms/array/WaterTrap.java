...
public class WaterTrap {
  ...
  /*
   ** Instructions to candidate. 1) Given an array of non-negative integers representing the
   * elevations from the vertical cross section of a range of hills, determine how many units of
   * snow could be captured between the hills.
   **
   ** See the example array and elevation map below. ___ ___ | | ___ | | ___ | |___ | | ___| | ___| |
   * | | | | | ___|___|___|___|___|___|___|___|___|___|___|___ {0, 1, 3, 0, 1, 2, 0, 4, 2, 0, 3, 0}
   ** ___ ___ | | ___ | | * * _*_ * | |_*_ * | | ___| | * _*_| | * | | | * | |
   ** ___|___|___|_*_|___|___|_*_|___|___|_*_|___|___ {0, 1, 3, 0, 1, 2, 0, 4, 2, 0, 3, 0}
   **
   ** Solution: In this example 13 units of snow (*) could be captured.
   ** 
   ** 2) Consider adding some additional tests in doTestsPass(). 3) Implement computeSnowpack()
   * correctly.
   */
  public static Integer computeSnowpack(Integer[] arr) {
    ...
    int[] maxSeenRight = new int[arr.length];// array that contains max height seen to the right of
                                             // tower i
    ...
    int maxSeenLeft = 0;// optimized to have a variable that contains max height of a tower
  ...
  private String getFirstDuplicateSequence(String input) {
    String end = input.split(".")[1];
    Set<String> set = new HashSet<String>();
    for (int i = 0; i + 4 < end.length(); i++) {
      String substr = end.substring(i, i + 4);
